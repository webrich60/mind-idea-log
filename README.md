# Life Compass Coach v4.5.1 プロフィール保護・復旧版

## v4.5.1の修正

- 起動時のGAS自動取得で、空のクラウドプロフィールが端末のプロフィールを上書きする不具合を修正
- 内容が空のプロフィールは、保存済みプロフィールより優先しない安全マージへ変更
- プロフィール専用の安全バックアップを端末内に自動保存
- 旧バックアップや退避データを探索し、最も内容が多いプロフィールを自動復旧
- `LifeCompass.restoreProtectedProfile()` による手動復旧にも対応
